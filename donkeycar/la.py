...
class Mat44(object):
    ...
    def inverse(self):
        ...
        # inverse(A) = adjunct(A) / det(A) （Aの逆行列は余因子行列を行列式で割ったもの）
        oodet = 1.0 / det
        ...
        # inverse(C) = -C * inverse(A)（Cの逆行列は -C × inverse(A)）
